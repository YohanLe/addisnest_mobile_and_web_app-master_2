...
router.use('/upload', (err, req, res, next) => {
  if (err && err.message && err.message.includes('Unexpected end of form')) {
    ...
    // If no files were processed, send a more helpful error
    console.log('No files found after busboy error - likely a client-side issue');
    return res.status(400).json({
      success: false,
      error: 'File upload incomplete. Please try uploading the file again.'
    });
  }
  next(err);
});
