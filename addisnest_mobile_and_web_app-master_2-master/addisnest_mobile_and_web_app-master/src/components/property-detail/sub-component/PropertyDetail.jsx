...
const PropertyDetail = ({ PropertyDetails, similarProperties }) => {
    ...
    // Memoize the similar properties to prevent unnecessary re-renders
    const memoizedSimilarProperties = useMemo(() => {
        return similarPropertiesFromRedux || [];
    ...
    useEffect(() => {
        ...
        if (initialRender.current) {
            ...
            if (memoizedSimilarProperties && memoizedSimilarProperties.length > 0) {
                setNearbyProperties(memoizedSimilarProperties);
        ...
        // Only update if memoized state changes, not when component re-renders
        if (memoizedSimilarProperties && memoizedSimilarProperties.length > 0) {
            setNearbyProperties(memoizedSimilarProperties);
        }
    }, [memoizedSimilarProperties, similarProperties]);
                        ...
                        <div style={{
                            ...
                            <button 
                                onClick={() => {
                                    ...
                                    const phoneInput = document.getElementById('schedule-phone-input');
                                    
                                    if (!dateInput || !timeSelect || !emailInput || !phoneInput) {
                                    ...
                                    const visitorEmail = emailInput.value;
                                    const visitorPhone = phoneInput.value;
                                    
                                    if (!selectedDate || !selectedTime || !visitorEmail) {
                                        alert('Please fill in all required fields (date, time, and email)');
                                        return;
                                    }
                                    ...
                                    // Create schedule status element if it doesn't exist
                                    let scheduleStatusElement = document.getElementById('schedule-status-element');
                                    if (!scheduleStatusElement) {
                                        scheduleStatusElement = document.createElement('div');
                                        scheduleStatusElement.id = 'schedule-status-element';
                                        scheduleStatusElement.style.marginTop = '15px';
                                        scheduleStatusElement.style.padding = '12px';
                                        scheduleStatusElement.style.borderRadius = '8px';
                                        scheduleStatusElement.style.fontSize = '14px';
                                        scheduleStatusElement.style.textAlign = 'center';
                                        
                                        // Insert after the schedule button
                                        const scheduleButton = document.activeElement;
                                        if (scheduleButton && scheduleButton.parentNode) {
                                            scheduleButton.parentNode.appendChild(scheduleStatusElement);
                                        }
                                    }
                                    
                                    // Show loading state
                                    scheduleStatusElement.style.backgroundColor = '#f0f7ff';
                                    scheduleStatusElement.style.color = '#0056b3';
                                    scheduleStatusElement.style.border = '1px solid #cce5ff';
                                    scheduleStatusElement.innerHTML = `
                                        <div style="display: flex; align-items: center; justify-content: center; gap: 10px;">
                                            <div style="width: 16px; height: 16px; border: 2px solid #0056b3; border-radius: 50%; border-top-color: transparent; animation: spin 1s linear infinite;"></div>
                                            <span>Sending tour request...</span>
                                        </div>
                                        <style>
                                            @keyframes spin {
                                                0% { transform: rotate(0deg); }
                                    ...
                                    // Create schedule through the proper API endpoint
                                    const sendScheduleRequest = async () => {
                                        try {
                                            ...
                                            const scheduleData = {
                                                ...
                                                visitorMessage: `Tour request for ${PropertyDetails?.title}`,
                                                visitorContact: {
                                                    email: visitorEmail,
                                                    phone: visitorPhone || '' // Include the phone number from the form
                                                }
                                            };
                                            
                                            // Send schedule request through the proper API endpoint
                                            const response = await Api.default.postWithtoken('schedules', scheduleData);
                                            ...
                                            // Show success message
                                            scheduleStatusElement.style.backgroundColor = '#d4edda';
                                            scheduleStatusElement.style.color = '#155724';
                                            scheduleStatusElement.style.border = '1px solid #c3e6cb';
                                            scheduleStatusElement.innerHTML = `
                                                <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                                                    <div style="font-size: 24px; margin-bottom: 5px;">✅</div>
                                                    <div style="font-weight: 600; margin-bottom: 5px;">Tour Request Sent!</div>
                                                    <div>Your ${selectedTourType === 'in-person' ? 'in-person' : 'video'} tour request has been created and emailed to the property owner.</div>
                                                    ...
                                                    <div style="font-size: 13px; margin-top: 5px; color: #666;">
                                                        The property owner will contact you at ${visitorEmail} to confirm.
                                                    </div>
                                                </div>
                                            `;
                                            ...
                                            // Remove the success message after 7 seconds
                                            setTimeout(() => {
                                                if (scheduleStatusElement) {
                                            ...
                                            scheduleStatusElement.style.color = '#721c24';
                                            scheduleStatusElement.style.border = '1px solid #f5c6cb';
                                            scheduleStatusElement.innerHTML = `
                                                <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                                                    <div style="font-size: 24px; margin-bottom: 5px;">❌</div>
                                                    <div style="font-weight: 600; margin-bottom: 5px;">Request Failed</div>
                                                    <div>${errorMessage}</div>
                                                    <button 
                                                        onclick="document.getElementById('schedule-status-element').style.display='none';"
                                                        style="margin-top: 10px; padding: 5px 10px; background-color: #f8d7da; border: 1px solid #721c24; border-radius: 4px; color: #721c24; cursor: pointer;"
                                                    >
                                                        Try Again
                                                    </button>
