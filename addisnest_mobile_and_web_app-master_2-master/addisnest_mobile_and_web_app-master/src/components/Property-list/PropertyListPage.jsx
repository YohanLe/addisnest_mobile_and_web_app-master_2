...
const PropertyListPage = ({ isHomePage = false, propertyCount, propertyType: homePagePropertyType }) => {
  ...
  const [userOwnProperties, setUserOwnProperties] = useState([]);
  ...
  useEffect(() => {
    ...
    // Only fetch user payments if user is properly authenticated with a valid token
    if (isLoggedIn) {
    ...
    // Fetch user's own properties if logged in
    if (isLoggedIn) {
      fetchUserOwnProperties();
    }
  ...
  const fetchMixedProperties = async () => {
    try {
      ...
      // Fetch ONLY ACTIVE properties for public viewing
      // This ensures pending properties are not shown to the public
      const response = await api.get('/properties', {
        params: {
          page: 1,
          limit: 50, // Get more properties to ensure we have variety
          status: 'active', // Only fetch approved/active properties
          // Don't filter by offeringType - get all active properties
        }
      });

      const allProperties = response.data?.data || [];
      if (process.env.NODE_ENV === 'development') {
        console.log('Total active properties fetched:', allProperties.length);
        console.log('Properties with status:', allProperties.map(p => ({ id: p._id, status: p.status, title: p.title })));
      }
      
      // Additional client-side filtering to ensure only active properties are shown
      const activeProperties = allProperties.filter(property => 
        property.status === 'active' || property.status === 'published'
      );
      
      if (process.env.NODE_ENV === 'development') {
        console.log('Filtered active properties:', activeProperties.length);
      }
      
      // Log property stats before deduplication
      logPropertyStats(activeProperties, 'before deduplication');
      
      // Remove duplicates based on _id
      const uniqueProperties = removeDuplicateProperties(activeProperties);
      
      if (process.env.NODE_ENV === 'development') {
        console.log('Unique active properties after deduplication:', uniqueProperties.length);
      }
  ...
  // Function to fetch user's own properties
  const fetchUserOwnProperties = async () => {
    ...
    try {
      ...
      const response = await api.getWithtoken(`properties/user/${tokenData.id}`);
      const userProperties = response.data || [];
      
      // Filter to show only pending properties (not yet approved by admin)
      const pendingProperties = userProperties.filter(property => 
        property.status === 'pending' || property.status === 'sold' || property.status === 'rented'
      );
      
      setUserOwnProperties(pendingProperties);
    } catch (error) {
      console.error('Error fetching user properties:', error);
    }
  };

  // Function to update property status (Sold/Rented)
  const updatePropertyStatus = async (propertyId, newStatus) => {
    try {
      setUpdatingStatus(prev => ({ ...prev, [propertyId]: true }));
      
      await api.putWithtoken(`properties/${propertyId}`, { status: newStatus });
      
      // Update the property in the state
      setUserOwnProperties(prevProperties => 
                ...
                <div key={`purchased-${property._id}`} className="col-lg-4 col-md-6" style={{ flex: '0 0 calc(33.333% - 20px)' }}>
                  ...
                  >
                    ...
                    <img 
                      ...
                      alt={property.title}
                      onError={(e) => {
                        // If image fails to load, use a different default image
                        const defaultImages = [Property1, Property2, Property3];
                        const randomIndex = Math.abs(property._id?.toString().charCodeAt(0) || 0) % defaultImages.length;
