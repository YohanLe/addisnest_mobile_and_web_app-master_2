    ...
    const Loginfun = async () => {
        ...
        if (errorMessage.isValid == false) {
            ...
            try {
                ...
                // The backend sends data directly, not wrapped in a data property
                const userData = response.data || response;
                console.log('Login response userData:', userData);
                
                // Store authentication data
                localStorage.setItem('addisnest_token', userData?.token);
                localStorage.setItem('isLogin', '1');
                localStorage.setItem("userId", userData?._id || userData?.user?.id || userData?.userId || userData?.id);
                localStorage.setItem("userType", activeTab);
                
                // Store comprehensive user data for immediate use
                if (userData) {
                    const userDataToStore = {
                        ...
                        email: userData.email || inps.email,
